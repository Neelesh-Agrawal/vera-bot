...
# Your API key (loaded from .env JUDGE_GROQ_API_KEY or GROQ_API_KEY)
LLM_API_KEY = ""

# Judge scoring model — separate from bot compose (GROQ_MODEL in .env).
LLM_MODEL = "llama-3.3-70b-versatile"

# For Ollama only: local server URL
OLLAMA_URL = "http://localhost:11434"

# Which test to run: "all" (replay, fast) | "phase2_short" | "full_evaluation" (slow)
TEST_SCENARIO = "all"

# Seconds between triggers in full_evaluation (bot + judge share one Groq key)
FULL_EVAL_SLEEP_SEC = 8
...
# Windows consoles default to cp1252 — score bars and LLM emoji crash without this
...
HTTP_USER_AGENT = "magicpin-judge-simulator/1.0"


def api_headers(extra: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    """Groq (and some APIs) return 403 for Python urllib's default User-Agent."""
...
def load_env_file() -> None:
    """Load .env into os.environ (simple parser, no extra deps)."""
...
def apply_config_from_env() -> None:
    """Override CONFIGURATION block from .env so keys aren't duplicated."""
    global LLM_API_KEY, LLM_MODEL, LLM_PROVIDER, BOT_URL
    load_env_file()
    if os.environ.get("BOT_URL"):
        BOT_URL = os.environ["BOT_URL"]
    if os.environ.get("JUDGE_GROQ_API_KEY"):
        LLM_API_KEY = os.environ["JUDGE_GROQ_API_KEY"]
    elif os.environ.get("GROQ_API_KEY"):
        LLM_API_KEY = os.environ["GROQ_API_KEY"]
    elif os.environ.get("ANTHROPIC_API_KEY"):
        LLM_API_KEY = os.environ["ANTHROPIC_API_KEY"]
    if os.environ.get("JUDGE_GROQ_MODEL"):
        LLM_MODEL = os.environ["JUDGE_GROQ_MODEL"]
...
class OpenAIProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None, *, json_mode: bool = False) -> str:
        ...
        req = urlrequest.Request(
            ...
            data=body,
            headers=api_headers({"Authorization": f"Bearer {self.api_key}", "Content-Type": "application/json"})
        )
...
class GeminiProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None, *, json_mode: bool = False) -> str:
        ...
        gen_config: Dict[str, Any] = {"temperature": 0.2, "maxOutputTokens": 1500}
        if json_mode:
        ...
        req = urlrequest.Request(url, data=body, headers=api_headers({"Content-Type": "application/json"}))
        resp = urlrequest.urlopen(req, timeout=TIMEOUT_LLM)
        data = json.loads(resp.read().decode("utf-8"))
...
class GroqProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None, *, json_mode: bool = False) -> str:
        ...
        for attempt in range(6):
            elapsed = time.time() - GroqProvider._last_request
            ...
            try:
                resp = urlrequest.urlopen(req, timeout=TIMEOUT_LLM)
                GroqProvider._last_request = time.time()
                data = json.loads(resp.read().decode("utf-8"))
...
class DatasetLoader:
    ...
    def load(self) -> bool:
        try:
            ...
            for seed_name, container, key, subdir in seed_specs:
                storage = getattr(self, container)
                seed_path = self.dataset_dir / seed_name
                if seed_path.exists():
...
class BotClient:
    ...
    def tick(self, triggers):
        # challenge-testing-brief.md allows up to 30s per tick; LLM batches need headroom
...
Score each dimension 0-10 with clear reasoning. Be STRICT."""
        ...
        for attempt in range(5):
            try:
                ...
                response = self.llm.complete(prompt, self.SYSTEM, json_mode=True)
                if response and response.strip():
                    return self._parse_response(response, action)
                print_warn("LLM returned empty response")
            except Exception as e:
                if attempt < 4 and ("429" in str(e) or "Too Many" in str(e)):
                    print_warn(f"LLM rate limited, retrying in {5 * (attempt + 1)}s...")
                    time.sleep(5 * (attempt + 1))
                    continue
                print_warn(f"LLM error: {e}")
                break

        print_warn("Using fallback score (25/50) — LLM scoring failed")
...
class JudgeSimulator:
    ...
    def run(self, scenario: str) -> bool:
        ...
        print_info(f"LLM: {self.llm.name() if self.llm else 'none (replay-only)'}")
    ...
    def _full(self) -> bool:
        ...
        for i in range(0, len(tids), 1):
            ...
            actions = data.get("actions", [])
            print_info(f"Trigger {i + 1}/{len(tids)}: {len(actions)} actions ({lat:.0f}ms)")

            for action in actions:
                self._score_and_display(action, verbose=False)

            time.sleep(FULL_EVAL_SLEEP_SEC)
    ...
    def _score_and_display(self, action: Dict, verbose: bool = True):
        ...
        if not self.scorer:
            ...
            print(f"\n{Colors.CYAN}Message:{Colors.RESET} \"{body}...\" (scoring skipped — no LLM)")
            return
...
def main():
    ...
    if replay_only:
        print_info("Replay-only mode — skipping LLM scoring (no API key needed)")
        judge = JudgeSimulator(None)
